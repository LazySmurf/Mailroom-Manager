...
{
    public partial class AddPBagForm : Form
    {
        ...
        public async void AddBag()
        {
            ...
            var BagContent = JsonConvert.SerializeObject(new Models.ParcelBag
            {
                BagNo = BagNoTxt.Text,
                ParcelNos = new List<string>()
            });
            var buffer = Encoding.UTF8.GetBytes(BagContent);
            var byteContent = new ByteArrayContent(buffer);
            byteContent.Headers.ContentType = new MediaTypeHeaderValue("application/json");
            var result = _httpClient.PostAsync(Settings.Default.API_Server + Global.PBagsURI, byteContent).Result;
            httpClient.Dispose();
        ...
        {
            InitializeComponent();
            MouseDown += AddPBagForm_MouseDown;
            MouseMove += AddPBagForm_MouseMove;
            TitleLabel.MouseDown += AddPBagForm_MouseDown;
            TitleLabel.MouseMove += AddPBagForm_MouseMove;
        ...
        private void AddPBag_Load(object sender, EventArgs e)
        {
            CurveBorder(20);
        ...
        //Move Form
        private void AddPBagForm_MouseDown(object sender, MouseEventArgs e)
        {
            MouseDownEvents(e);
        }

        private void AddPBagForm_MouseMove(object sender, MouseEventArgs e)
        {
            MouseMoveEvents(e);
        }
